...
@section('content')
    <div class="container-fluid px-3 mt-3 bg-body-tertiary flex-grow-1">
        <h2 class="my-4">Subcategories</h2>
        <div class="card">
            <div class="card-body">
                <table id="tablesubcategories" class="table table-bordered table-hover dataTable dtr-inline">
                    <thead>
                        <tr>
                            <th>Name</th>
                            <th>Description</th>
                    ...
                    <tbody>
                        @foreach ($subcategories as $item)
                            <tr @if($item->deleted_at) style="background-color: lightgray;" @endif>
                                ...
                                <td>
                                    <form action="{{ route('admin.master.subcategories.edit', ['id' => $item->id]) }}" method="get">
                                ...
                                <td>
                                    @if ($item->deleted_at)
                                        <form action="{{ route('admin.master.subcategories.activate') }}" method="post">
                                            @csrf
                                            <input type="hidden" name="id" value="{{$item->id}}">
                                            <button type="submit" class="btn btn-success w-100">Activate</button>
                                        </form>
                                    @else
                                        <form action="{{ route('admin.master.subcategories.deactivate') }}" method="post">
                                            @csrf
                                            <input type="hidden" name="id" value="{{$item->id}}">
